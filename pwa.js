...
const { Wallet, initPugdagFramework, log } = require('@pugdag/wallet-worker');
const { RPC } = require('@pugdag/grpc-node');
const { dpc } = require('@pugdag/wallet/dist/utils/helper');

class PugdagPWA extends EventEmitter {
	constructor(appFolder){
		super();
		this.appFolder = appFolder;
		this.config = utils.getConfig(path.join(appFolder, "config", "pugdag-wallet-pwa"));
	...
	async initHttp(){
		...
		const flowHttp = this.flowHttp = new FlowHttp(__dirname, {
			config:{
				...
				grpc:{
					protoPath: path.join(__dirname, 'node_modules', '@pugdag', 'grpc', 'proto', 'messages.proto'),
		...
		flowHttp.on("app.init", async (args)=>{
			...
			const {
				pugdagUX='node_modules/@pugdag/ux',
				flowUX='node_modules/@aspectron/flow-ux',
				walletWorker='node_modules/@pugdag/wallet-worker',
				secp256k1='node_modules/secp256k1-wasm/http',
				grpcWeb='node_modules/@pugdag/grpc-web',
				flowGRPCWeb='node_modules/@aspectron/flow-grpc-web',
				pugdagCoreLib='node_modules/@pugdag/core-lib'
			} = folders;
			...
			const files = [
				'./',
				flowUX,pugdagUX,grpcWeb,
				'node_modules/@pugdag/wallet',
				'node_modules/@pugdag/grpc',
				pugdagCoreLib
				...
				return new Promise((resolve, reject) => {
					...
					try {
						const files = [
							...
							path.resolve(__dirname, 'node_modules/@aspectron/flow-ux/package.json'),
							path.resolve(__dirname, 'node_modules/@pugdag/node-pugdag-ux/package.json'),
							path.resolve(__dirname, 'node_modules/@pugdag/grpc-web/package.json'),
							path.resolve(__dirname, 'node_modules/@pugdag/wallet/package.json'),
							path.resolve(__dirname, 'node_modules/@pugdag/grpc/package.json'),
							path.resolve(__dirname, 'node_modules/@pugdag/core-lib/package.json'),
							path.resolve(__dirname, 'node_modules/@pugdag/wallet-worker/package.json')
						];
			
						let list = files.map(file => {
							if (fs.existsSync(file)) {
								const { version, name } = JSON.parse(fs.readFileSync(file, 'utf8'));
								console.log(`[version]: (${version}) for: ${file}`);
								return { version, name };
							} else {
						...
						fs.readFile(indexFile, { encoding: 'utf-8' }, (err, data) => {
							if (err) {
								...
								let indexHtml = data.replace(
									`<script type="module" src="/dist/wallet-app.js"></script>`,
									`\n${script}\n\t<script type="module" src="/dist/wallet-app.js?v=${hash}"></script>`
								);
								indexHtml = indexHtml.replace('ident:"pugdag:ident"', `ident:"${hash}"`);
								resolve(indexHtml); // Assuming you want to do something with the modified HTML
							}
						});
					} catch (ex) {
						console.error('Error in updateIndex:', ex);
						reject(ex);
					}
				});
			}
			...
			//pugdag-wallet-worker/worker.js
			app.use('/resources', express.static( path.join(pugdagUX, "resources"), {
				index: 'false'
			}))
			...
			app.get('/api/health', async (req, res)=>{
				...
				let isConnected = this.grpc.pugdag.client.isConnected;
				if (tsDiff < 20){
					res.status(504).send(JSON.stringify({code:"PLEASE-WAIT-20-SEC-FOR-BLOCK-INFO-REQUEST", isConnected}))
					return
				}

				info = await this.grpc.pugdag.request('getInfoRequest')
					.then(i=>{
						if (!i?.isSynced){
							status = 502;
						}
			...
			app.get('/pugdag-wallet-worker/worker.js', (req, res)=>{
				res.sendFile(path.join(rootFolder, 'dist/pugdag-wallet-worker-core.js'))
			})
			...
			app.get('(/pugdag-wallet-worker)?/secp256k1.wasm', (req, res)=>{
			...
			let router = new FlowRouter(app, {
				...
				folders:[
					...
					{url:'/pugdag-ux', folder:pugdagUX},
					{url:'/node_modules/@aspectron/flow-ux', folder:flowUX},
					{url:'/pugdag-wallet-worker', folder:walletWorker},
					{url:'/resources/extern', folder:flowUX+'/resources/extern'},
					{url:'/@pugdag/grpc-web', folder:grpcWeb},
	...
	async initPugdag() {
		...
		let network = filter.shift() || 'pugdag';
		...
		const pugdag = new RPC({ clientConfig:{ host } });
		pugdag.onError((error)=>{ log.error(`gRPC[${host}] ${error}`); })
		pugdag.onConnect(async()=>{
			let res = await pugdag.getUtxosByAddresses([])
			.catch((err)=>{
				//error = err;
			})
		...
		this.grpc = { network, port, host, pugdag, flags:{} }
	}
	...
	async initMonitors() {
		...
		const poll = async () => {
			if(!this.grpc.pugdag.client.isConnected)
			...
			try {
				const bdi = await  this.grpc.pugdag.request('getBlockDagInfoRequest');
				const vspbs = await  this.grpc.pugdag.request('getVirtualSelectedParentBlueScoreRequest');
	...
	*/
	...
	async initRPC() {
		...
		const faucetUrl = 'https://faucet.pugdag-net.com';
	...
	async main() {
		...
		program
			...
			.description('Pugdag Node Wallet')
			...
			.option('--mainnet','use pugdag/mainnet network')
			...
			.action(async ()=>{
				...
				if(this.options.restartAfter){
					...
					setTimeout(async ()=>{
						console.log("::::RESTART::::")
						process.exit(0);
						//pugdagPWA.flowHttp.server.close(()=>{
...
(async () => {
	let pugdagPWA = new PugdagPWA(__dirname);
	try {
		await pugdagPWA.main();
	} catch(ex) {
		console.log(ex.toString());
	}
})();
